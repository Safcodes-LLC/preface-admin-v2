...
import { Tabs, Form, Button, message, Modal, Select, Input, Upload, Radio } from 'antd';
import { PlusOutlined, LoadingOutlined, MenuFoldOutlined, MenuUnfoldOutlined } from '@ant-design/icons';
import Flex from 'components/shared-components/Flex';
import GeneralField from './GeneralField';
import SeoField from './SeoField';
import { editPost, updateStatus } from 'store/slices/postSlice';
	...
	const onFinish = async (saveType) => {
		...
		try {
			...
			if (mode === EDIT) {
				...
				try {
					const response = await axios.put(`${API_BASE_URL}/pages/${id}`, pageUpdateData, {
						headers: {
							'Content-Type': 'application/json',
				...
				<PageHeaderAlt className="border-bottom" overlap>
					<div className="container">
						<Flex className="py-2" mobileFlex={false} justifyContent="space-between" alignItems="center">
							{view ? (
								<h2 className="mb-3">View Page</h2>
							) : (
								<>
									<h2 className="mb-3">{mode === 'ADD' ? 'Add New Page' : `Edit Page`} </h2>
									<div className="mb-3">
										{editOn ? (
										...
										{(!editOn || userRoles.includes('Administrator') || mode === 'ADD') && (
											<Button
												type="primary"
												style={{ padding: '0px 30px', textTransform: 'uppercase', borderRadius: '8px' }}
