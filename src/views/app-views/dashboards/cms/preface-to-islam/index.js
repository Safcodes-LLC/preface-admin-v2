...
const PrefaceToIslam = () => {
      ...
      const fetchListPosts = async () => {
        try {
          const response = await axios.get(
            `${API_BASE_URL}/frontend/list-posts?limit=100`
          );
  ...
  useEffect(() => {
    const fetchAuthors = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/frontend/all-authors`, {
          headers: {
  ...
  //get data listmain
  useEffect(() => {
    // Check if the pathname includes 'preface-to-islam'
    const isPrefaceToIslam = location.pathname.includes("preface-to-islam");

    const fetchFeaturedPosts = async () => {
      if (!isPrefaceToIslam) return; // Exit if the path is not 'preface-to-islam'

      try {
        const token = localStorage.getItem("auth_token"); // Get the token from local storage
        const response = await axios.get(
          `${API_BASE_URL}/posts/preface-featured`, // Use the appropriate API URL
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: token, // Pass the token directly without 'Bearer'
  ...
  const handleCheckboxChange = (record) => {
    ...
    togglePrefaceFeatured(
      record._id,
      featuredStatusChanges[record._id] ?? record.prefaceFeatured
    );
  };
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      const updatedList = allListData.map((item) => {
        const newStatus = featuredStatusChanges[item._id];
        if (newStatus !== undefined) {
          return { ...item, prefaceFeatured: newStatus };
        }
      ...
      message.success("prefaceFeatured statuses updated successfully");
      ...
      message.error("Failed to update prefaceFeatured statuses");
    } finally {
  ...
  const togglePrefaceFeaturedAPI = async (postId, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/posts/${postId}/toggle-prefacefeatured`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({ prefaceFeatured: newStatus }),
        }
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to update prefaceFeatured status"
        );
      }
    } catch (error) {
  ...
  const togglePrefaceFeatured = (postId, currentStatus) => {
    ...
    // console.log(`Post ID: ${postId}, prefaceFeatured Status: ${newStatus}`);
  ...
  const deleteRow = async (row) => {
    ...
    try {
      // API request to set prefacefeatured to false
      const response = await axios.put(
        `${API_BASE_URL}/posts/${row._id}/toggle-prefacefeatured`,
        { prefaceFeatured: false },
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: token,
      ...
      if (response.status === 200) {
        ...
        const updatedList = listMain.map((item) =>
          item._id === row._id ? { ...item, prefaceFeatured: false } : item
        );
        setListMain(updatedList);
        message.success(
          "Post removed successfully and set to not prefaceFeatured."
        );
        window.location.reload();
      } else {
        throw new Error("Failed to update prefaceFeatured status");
      }
    } catch (error) {
