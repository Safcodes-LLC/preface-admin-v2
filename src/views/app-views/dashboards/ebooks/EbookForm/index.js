...
const EbookForm = (props) => {
  ...
  const ebooks_list = useSelector((state) => state.post.posts);
  useEffect(() => {
    if (!ebooks_list.length && mode === EDIT && !isPostsFetched) {
      dispatch(
  ...
  useEffect(() => {
    if (mode === EDIT && !loading) {
      ...
      setThumbnailImage(articleData.pdf_file);
      setAllSelectedThumbnailImgs([articleData.pdf_file]);
  ...
  const onFinish = () => {
    ...
    form
      .validateFields()
      .then((values) => {
        values.postType = "66d9d564987787d3e3ff1315";
        values.pdf_file = uploadedThumbnailImg;
          ...
          if (mode === ADD) {
            ...
            dispatch(createPost({ postData: values })).then((result) => {
              if (result.type.includes("rejected")) {
                ...
                navigate(`/admin/dashboards/ebooks/listing`);
              }
            });
            message.success(`Ebook ${values.title} is created successfully`);
          }
          if (mode === EDIT) {
            ...
            dispatch(updatePost({ postData: values, postId: id })).then(
              (result) => {
                if (result.type.includes("rejected")) {
                  ...
                  setAllSelectedThumbnailImgs([result.payload.pdf_file]);
      ...
      >
        <PageHeaderAlt className="border-bottom" overlap>
          <div className="container">
            ...
            >
              {view ? (
                <h2 className="mb-3">View E-book</h2>
              ) : (
                <>
                  <h2 className="mb-3">
                    {mode === "ADD" ? "Add New E-book" : `Edit E-book`}{" "}
                  </h2>
                  <div className="mb-3">
                    {editOn ? (
                      <Button
