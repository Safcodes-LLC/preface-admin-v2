...
const EbookList = () => {
  ...
  useEffect(() => {
    // Dispatch the action to fetch Ebook posts
    dispatch(
  ...
  const allEbookPosts = useSelector((state) => state.post.posts);
  ...
  const AddEbook = () => {
    navigate(`/admin/dashboards/ebooks/add-ebook`);
  };

  const viewDetails = (row) => {
    navigate(`/admin/dashboards/ebooks/view-ebook/${row._id}`);
  };

  const editDetails = (row) => {
    navigate(`/admin/dashboards/ebooks/edit-ebook/${row._id}`);
  };
