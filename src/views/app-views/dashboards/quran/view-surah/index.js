...
const ViewSurah = () => {
  ...
  //   navigate(
  //     `/admin/dashboards/quran/view-surah/${surahId}/ayah/${record.ayahNumber}`
  //   );
  // };
  ...
  const editDetails = (record) => {
    navigate(`/admin/dashboards/quran/view-surah/${surahId}/edit-ayah/${record.ayahNumber}`);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return <Tag style={{ fontSize: 12 , padding: "0px 4px", textTransform:'capitalize'}} color="orange">Ungrouped</Tag>;
      },
    },
    // {
    //   title: "Action",
  ...
  return (
    <Card loading={loading}>
        ...
        <div>
          ...
          {surahInfo && (
            <Space size={12} style={{ marginTop: 8 }}>
              ...
              <Tag
                color={surahInfo.revelationPlace === "makkah" ? "gold" : "cyan"}
              >
                {surahInfo.revelationPlace?.[0]?.toUpperCase() +
                  surahInfo.revelationPlace?.slice(1)}
              </Tag>
              {surahInfo.pages && (
                <Tag>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button
            key="publish"
            type="primary"
            onClick={() => message.success("Surah info published!")}
          >
            Publish
          </Button>,
          <Button
            key="save"
            onClick={() => message.success("Surah info saved!")}
          >
            Save Draft
          </Button>,
        ]}
      ...
      <Modal
        title="Create Ayah Group"
        open={groupModalVisible}
        onOk={handleConfirmGroup}
        ...
        <Form layout="vertical">
          <Form.Item label="Selected Ayahs for Grouping">
            ...
            >
              <p>
                <strong>Ayahs:</strong>{" "}
                {selectedAyahs.sort((a, b) => a - b).join(", ")}
              </p>
              <p>
                <strong>Group Range:</strong> {Math.min(...selectedAyahs)} -{" "}
                {Math.max(...selectedAyahs)}
              </p>
              <p>
          ...
          <Form.Item>
            <p style={{ color: "#666", fontSize: "12px" }}>
              These ayahs will be grouped together. When viewing any ayah in
              this group, the translation and tafsir will apply to the entire
              group.
            </p>
          </Form.Item>
        </Form>
