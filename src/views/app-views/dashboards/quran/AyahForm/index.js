...
const AyahForm = ({ mode = "EDIT", view = false }) => {
  ...
  // Determine if component is in view mode
  ...
  useEffect(() => {
    ...
    const currentTafsir = tafsirs.find(
      (t) => t.languageCode === selectedLanguage
    );
  ...
  const handlePreviousAyah = () => {
    ...
    if (prevAyahNumber >= 1) {
      const url = isViewMode
        ? `/admin/dashboards/quran/view-surah/${surahId}/ayah/${prevAyahNumber}`
        : `/admin/dashboards/quran/view-surah/${surahId}/edit-ayah/${prevAyahNumber}`;
      navigate(url);
    } else {
  ...
  const handleNextAyah = () => {
    ...
    if (nextAyahNumber <= totalAyahs) {
      const url = isViewMode
        ? `/admin/dashboards/quran/view-surah/${surahId}/ayah/${nextAyahNumber}`
        : `/admin/dashboards/quran/view-surah/${surahId}/edit-ayah/${nextAyahNumber}`;
      navigate(url);
    } else {
    ...
    <Card>
      ...
      >
        <Button
          ...
          onClick={() =>
            navigate(`/admin/dashboards/quran/view-surah/${surahId}`)
          }
        >
          Back to Surah
        </Button>
      ...
      {ayahData && (
        <>
          ...
          >
            <div>
              <Title level={3} style={{ marginBottom: 8 }}>
                {ayahData.surah?.nameComplex} ({ayahData.surah?.nameArabic}) -
                Ayah {ayahNumber}
              </Title>
              <Space>
                ...
                {ayahData.ayahGroupStart === null &&
                  ayahData.ayahGroupEnd === null && (
                    <Tag color="orange">
            ...
            <Form.Item label="Ayah Information">
              <textarea
                ...
                style={{
                  ...
                  backgroundColor: isViewMode ? "#f5f5f5" : "transparent",
            ...
            >
              <textarea
                ...
                placeholder={`Enter translation for ${selectedLanguage}${
                  ayahData.ayahGroupStart === null &&
                  ayahData.ayahGroupEnd === null
                    ? ` (unified translation for ayahs ${ayahData.ayahGroupStart}-${ayahData.ayahGroupEnd})`
                    : ""
                }`}
                readOnly={isViewMode}
                style={{
          ...
          <Form layout="vertical">
            <Form.Item
              label={`Tafsir (${selectedLanguage.toUpperCase()})${
                ayahData.ayahGroupStart === null &&
                ayahData.ayahGroupEnd === null
                  ? ` - Group ${ayahData.ayahGroupStart}:${ayahData.ayahGroupEnd}`
                  : ""
            ...
            >
              <textarea
                ...
                placeholder={`Enter tafsir for ${selectedLanguage}${
                  ayahData.ayahGroupStart === null &&
                  ayahData.ayahGroupEnd === null
                    ? ` (unified tafsir for ayahs ${ayahData.ayahGroupStart}-${ayahData.ayahGroupEnd})`
                    : ""
                }`}
                readOnly={isViewMode}
                style={{
