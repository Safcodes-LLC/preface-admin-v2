...
const BlogList = () => {
  ...
  useEffect(() => {
    // Dispatch the action to fetch Blog posts
    dispatch(
  ...
  const allBlogPosts = useSelector((state) => state.post.posts);
  const [list, setList] = useState([]);
  const [selectedRows, setSelectedRows] = useState([]);
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);

  useEffect(() => {
    setList(allBlogPosts);
  ...
  const AddBlog = () => {
    navigate(`/admin/dashboards/blogs/add-blog`);
  };

  const viewDetails = (row) => {
    navigate(`/admin/dashboards/blogs/view-blog/${row._id}`);
  };

  const editDetails = (row) => {
    navigate(`/admin/dashboards/blogs/edit-blog/${row._id}`);
  };
  ...
  const deleteRow = (row) => {
    ...
    if (selectedRows.length > 1) {
      selectedRows.forEach((elm) => {
        ...
        setList(data);
        setSelectedRows([]);
        // Need to dispatch the delete Post
