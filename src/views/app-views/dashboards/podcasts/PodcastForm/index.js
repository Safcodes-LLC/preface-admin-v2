...
const PodcastForm = (props) => {
  ...
  useEffect(() => {
    if (mode === EDIT && !loading) {
      ...
      setThumbnailImage(articleData.audio_file);
      setAllSelectedThumbnailImgs([articleData.audio_file]);
  ...
  const onFinish = () => {
    ...
    form
      ...
      .then((values) => {
        ...
        values.postType = "66d9d564987787d3e3ff1313";
        values.audio_file = uploadedThumbnailImg;
          ...
          if (mode === ADD) {
            ...
            dispatch(createPost({ postData: values })).then((result) => {
              if (result.type.includes("rejected")) {
                ...
                navigate(`/admin/dashboards/podcasts/listing`);
              }
            });
            message.success(`Podcast ${values.title} is created successfully`);
          }
          if (mode === EDIT) {
            // call API to Update a Podcast
            const { id } = param;
            dispatch(updatePost({ postData: values, postId: id })).then(
              (result) => {
                if (result.type.includes("rejected")) {
                  ...
                  setAllSelectedThumbnailImgs([result.payload.audio_file]);
      ...
      >
        <PageHeaderAlt className="border-bottom" overlap>
          <div className="container">
            ...
            >
              {view ? (
                <h2 className="mb-3">View Podcast</h2>
              ) : (
                <>
                  <h2 className="mb-3">
                    {mode === "ADD" ? "Add New Podcast" : `Edit Podcast`}{" "}
                  </h2>
