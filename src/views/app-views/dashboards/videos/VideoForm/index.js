...
// Block style function to handle text alignment and indentation and line spacing
...
const VideosForm = (props) => {
  ...
  useEffect(() => {
    if (mode === EDIT && !loading && list.length > 0) {
      ...
      setEditorState(
        EditorState.createWithContent(convertFromRaw(JSON.parse(mainContent)), createDecorator())
      );
            ...
            dispatch(updatePost({ postData: updateValue, postId: id })).then(
              (result) => {
                if (result.type.includes("rejected")) {
                  ...
                  message.error(result.payload || 'Failed to update video');
                } else {
